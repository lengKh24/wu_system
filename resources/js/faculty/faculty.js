...
(() => {
    ...
    const CONFIG = {
        ...
        BULK_DESTROY_URL: '/api/v1/faculties-bulk-destroy',
        DEBOUNCE_DELAY: 300,
        LOCALE: 'en-GB'
    };
    ...
    const DOM = {
        ...
        destroyAllBtn: document.getElementById('facultyDestroyAllBtn'),
...
function renderTable(facultys) {
    ...
    // Permanent hard-delete of every faculty — bypasses soft-delete
    // entirely, so this asks for a typed confirmation rather than a plain
    // OK/Cancel dialog. WARNING: cascades to delete every major/subject
    // under each faculty too (DB-level cascadeOnDelete).
    async function handleDestroyAll() {
        const confirmation = await Swal.fire({
            title: 'លុបមហាវិទ្យាល័យទាំងអស់ជាអចិន្ត្រៃយ៍?',
            html: 'សកម្មភាពនេះមិនអាចត្រឡប់វិញបានទេ។ វាយ <b>DELETE</b> ដើម្បីបញ្ជាក់។<br><span class="text-xs text-neutral-400">This permanently deletes ALL faculties (and cascades to their majors/subjects) and cannot be undone. Type DELETE to confirm.</span>',
            icon: 'warning',
            input: 'text',
