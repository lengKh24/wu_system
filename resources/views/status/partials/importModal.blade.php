<x-ui.modal id="statusImportModal" card-id="statusImportModalCard" title-id="statusImportModalTitle"
    title="នាំចូលស្ថានភាព (Import Statuses)" form-id="statusImportForm"
    close-fn="StatusImportModal" max-width="max-w-md">
    ...
    <div>
        ...
        <div id="statusImportDropzone" tabindex="0"
        ...
        <input type="file" id="statusImportFile" accept=".xlsx,.xls,.csv" class="hidden">
        <div class="flex items-center justify-between mt-1.5">
            <p id="statusImportFileName" class="text-xs text-neutral-400 truncate"></p>
            <button type="button" id="statusImportClearBtn" class="hidden text-xs font-semibold text-rose-500 hover:text-rose-600">
                លុបឯកសារ (Remove)
            </button>
        </div>
    </div>
    ...
    <x-slot:footer>
        <button type="button" onclick="StatusImportModal.toggle(false)"
        ...
        <button type="submit" form="statusImportForm" id="statusImportSubmitBtn"
            ...
            <svg id="statusImportSpinner" class="hidden w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
            </svg>
            <span id="statusImportSubmitLabel">នាំចូល (Import)</span>
        </button>
    </x-slot:footer>
</x-ui.modal>

{{-- Import Results — categorized, readable tables instead of a raw dump. --}}
<x-ui.modal id="statusImportResultsModal" card-id="statusImportResultsModalCard" title-id="statusImportResultsModalTitle"
    title="លទ្ធផលនាំចូល (Import Results)" close-fn="StatusImportResultsModal" max-width="max-w-2xl">
    <div id="statusImportResultsBody" class="space-y-5"></div>

    <x-slot:footer>
        <button type="button" onclick="StatusImportResultsModal.toggle(false)"
